...
bfcp_message *bfcp_build_message(bfcp_arguments* arguments)
{
	...
	switch(arguments->primitive) {
        ...
        case FloorRequestStatusAck:
            return bfcp_build_message_FloorRequestStatusAck(arguments->entity, arguments->fID, arguments->bID, arguments->pInfo, arguments->priority);
		case Error:
...
bfcp_message *bfcp_build_message_FloorRequestStatusAck(bfcp_entity *entity, bfcp_floor_id_list *fID, unsigned short int bID, char *pInfo, unsigned short int priority)
{
    int err;
    bfcp_message *message = bfcp_new_message(NULL, 0);
    if(!message)	/* We could not allocate the memory, return a with failure */
    ...
    /* Append the attributes buffer to the common header one */
    bfcp_build_commonheader(message, entity, FloorRequestStatusAck);
    return message;
}