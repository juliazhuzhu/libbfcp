...
void menu(char *lineptr)
{
	...
	while(fgets(line, 79, stdin) != NULL) {
		...
		switch(*lineptr) {
			...
			case 'h':
				++lineptr;
				if(bfcp_destroy_bfcp_participant(&list, context) < 0)
			...
			case 'r':
            {
                ...
                char info[128] = {0};
                unsigned long participant = 0;
                error = scanf (" %d[^\n]", &participant);
                sprintf(info, "%lu", participant);
                text = (char*)malloc(sizeof(info));
                memset(text,0, sizeof(info));
                memcpy(text, info, sizeof(info) - 1);
				...
				error = bfcp_floorRequest_participant(list, beneficiaryID, priority, node, NULL,context);
			...
			case 'l':
				...
				BFCP_CHECK_INTEGER;
				error = bfcp_floorRelease_participant(list, floorRequestID,context);
				BFCP_CHECK_MESSAGE;
			...
			case 'o':
				...
				error = scanf ("%hu", &floorRequestID);
				BFCP_CHECK_INTEGER;
				error = bfcp_floorRequestQuery_participant(list, floorRequestID,context);
				BFCP_CHECK_MESSAGE;
			...
			case 'u':
				...
				error = scanf ("%u", &beneficiaryID);
				BFCP_CHECK_INTEGER;
				error = bfcp_userQuery_participant(list, beneficiaryID,context);
				BFCP_CHECK_MESSAGE;
			...
			case 'e':
				...
				error = bfcp_floorQuery_participant(list, node,context);
				if(error == -1)
					remove_floor_list_p(node);
				BFCP_CHECK_MESSAGE;
			...
			case 'a':
				...
				BFCP_CHECK_INTEGER;
				error = bfcp_chairAction_participant(list, floorRequestID, text, status, list_floor, queue_position,context);
				free(text);
				if(error == -1)
					remove_floor_list_p(node);
				BFCP_CHECK_MESSAGE;
			...
			case 'f':
				++lineptr;
				error = bfcp_hello_participant(list,context);
				BFCP_CHECK_MESSAGE;
